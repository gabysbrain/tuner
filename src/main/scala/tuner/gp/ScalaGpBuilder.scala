 ...
 */
object ScalaGpBuilder extends GpBuilder with LazyLogging {
  ...
  def minDist(samples:DenseMatrix[Double]) : Double = {
    ...
    for(r1 <- 0 until (samples.rows-1)) {
      ...
      for(r2 <- (r1+1) until samples.rows) {
        ...
        logger.trace("r1 " + row1 + " r2 " + row2 + " d " + dist)
        if(dist < md) {md = dist}
      }
    }
    math.sqrt(md)
  ...
  def startParams(samples:DenseMatrix[Double], retries:Int=5)
    ...
    logger.debug("md " + md + " mn " + minCorr + " mx " + maxCorr)
    List.fill(retries) {
  ...
  def findParams(samples:DenseMatrix[Double], responses:DenseVector[Double],
    ...
    val results:List[Try[(Double,DenseVector[Double])]] =
      ...
      Try({
        ...
        */
        //val ll = optimFunc(pt)
        logger.debug("optim result: " + pt + " " + results.value)
        logger.trace("took " + results.iter + " iterations")
        logger.trace("success? " + !results.searchFailed)
        logger.trace("reason: " + results.convergedReason)
        (results.value, pt)
      })
    }
    logger.info("finished with optimizations")

    logger.trace("all res " + results)
